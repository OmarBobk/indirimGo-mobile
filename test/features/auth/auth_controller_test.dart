...
void main() {
  test('restores an existing session without exposing login state', () async {
    ...
    expect(container.read(authControllerProvider).phase, AuthPhase.initializing);
    await _settle();
  ...
  test('login transitions through the two-factor challenge and authenticates', () async {
    final repository = FakeAuthRepository()
  ...
  test('recovery completion is supported and fresh login drops challenge', () async {
    var suppliedRecovery = '';
    final repository = FakeAuthRepository()
      ..loginHandler = (_, __) async =>
          LoginTwoFactorRequired(sampleChallenge)
      ..recoveryHandler = (_, recovery) async {
  ...
  test('expired challenge stays local and asks for a fresh login', () async {
    ...
    final repository = FakeAuthRepository()
      ...
      ..authenticatorHandler = (_, __) async {
        completions += 1;
        return sampleSession;
      };
    final container = _container(repository);
  ...
  test('failed logout restores authenticated state for retry', () async {
    ...
    expect(container.read(authControllerProvider).phase, AuthPhase.authenticated);
    expect(container.read(authControllerProvider).error?.kind, ApiErrorKind.network);
    container.dispose();
  });
}
